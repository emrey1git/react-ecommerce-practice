    ...
    <>
      <PageContainer>
        ...
        <Drawer anchor="right" open={isBasketOpen} onClose={basketClose}>
          {/* Genişlik ve Padding */}
          <div className="basket-drawer-container">
            {/* Başlık ve Kapatma Butonu */}
            <div className="basket-header">
              <h3>Sepetim ({products.length})</h3>
              <button className="basket-close-btn" onClick={basketClose}>
                X
              </button>
            </div>
            ...
            {products.length === 0 ? (
              ...
              products
                .filter(
                  (product) => product && product.title && product.price > 0
                )
                .map((product, index) => {
                  return (
                    <div className="basket-item" key={product.id || index}>
                      <img
                        src={product.image}
                        alt={product.title}
                        width={50}
